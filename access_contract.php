<?php
  ...
  if (count($places)!=0){
    $query.=" idFlight =".$_COOKIE['nowId']." AND ";
  $query2.=" idFlight =".$_COOKIE['nowId']." AND ";
  ...
  $result2=mysqli_query($link2,$query2)or die("Ошибка запроса2".mysqli_error($link2));
...
<body>
  ...
  <div class="place_form">
    ...
    for ($i=0; $i < mysqli_num_rows($result); $i++) {
      ...
      echo "<p class=''>Билеты не выбраны</p>";
   }
echo "</div>";
    ?>
    ...
    <form class="form_class" method="post" action="check_List.php">
      ...
      <div id='serial_par' class='sel'>
        <input type="text" id='serial' name='serial' class='input_place' placeholder="Серия паспорта" pattern="[0-9]{4}" title='Серия паспорта должна содержать 4 цифры.' maxlength="4" required>
      </div>
      ...
      <?php
      if (count($places)!=0){
        echo "<input type='submit' id='finish_btn' lign='center' class='finish_btn' value='Подтвердить заказ'>";
      }
       ?>
